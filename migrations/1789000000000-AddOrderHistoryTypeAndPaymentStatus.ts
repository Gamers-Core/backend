...
export class AddOrderHistoryTypeAndPaymentStatus1789000000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "order_status_history_type_enum" AS ENUM ('status', 'payment_status')`);
        await queryRunner.query(`ALTER TABLE "order_status_history" ADD "type" "order_status_history_type_enum"`);
        await queryRunner.query(`CREATE TYPE "order_status_history_status_enum" AS ENUM ('pending','confirmed','on-hold','on-progress','shipped','delivered','completed','returned','cancelled','unpaid','paid','refunded')`);
