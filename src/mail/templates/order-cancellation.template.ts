import { MailTemplateFn } from '../types';

export const renderOrderCancellationHtml: MailTemplateFn<'order_cancellation'> = (
  t,
  { orderNumber, currency, total, items },
  isRtl,
) =>
  `
  <h2 style="margin-bottom: 0;">🎮 Gamers Core</h2>
  <p style="margin-top: 4px; color: #555;">${t('mail.orderCancellation.header')}</p>

  <hr style="margin: 20px 0;" />

  <p>${t('mail.common.greeting')}</p>
  <p>${t('mail.orderCancellation.thanksReceipt')}</p>

  <div style="background: #f5f5f5; padding: 12px; border-radius: 8px;">
    <strong>${t('mail.common.orderNumber')}</strong> ${orderNumber}<br />
    <strong>${t('mail.common.total')}</strong> ${currency}${total}
  </div>

  <h3 style="margin-top: 20px;">${t('mail.orderCancellation.summary')}</h3>

  <table width="100%" cellspacing="0" cellpadding="8" style="border-collapse: collapse;">
    <thead>
      <tr style="background: #eee; text-align: ${isRtl ? 'right' : 'left'};">
        <th>${t('mail.common.item')}</th>
        <th>${t('mail.common.variant')}</th>
        <th>${t('mail.common.qty')}</th>
        <th>${t('mail.common.total')}</th>
      </tr>
    </thead>
    <tbody>
      ${items
        .map(
          (item) => `
      <tr>
        <td>${item.productTitle}</td>
        <td>${item.variantName}</td>
        <td>${item.quantity}</td>
        <td>${currency}${item.lineTotal}</td>
      </tr>
      `,
        )
        .join('')}
    </tbody>
  </table>

  <hr style="margin: 20px 0;" />

  <p><strong>${t('mail.common.total')}</strong> ${currency}${total}</p>

  <p style="color: #555;">${t('mail.orderCancellation.shippedNotice')}</p>
  `;
